...
// the chance rates for each rarity within range and use that
// as total chance. the % chance is then currentchance/totalchance
function getRarityChance(min, max, rarity) {
  var total_chance = 0;
  if (rarity >= min && rarity <= max) {
    ...
    console.log("INDIV rare: " + rarity + " with min " + min + " and max " + max + " is " + Number(chanceRates.rarity[rarity]/total_chance));
    return Number(chanceRates.rarity[rarity]/total_chance);
  }
  return 0;
}

// follows similar logic to the pull rate shit go read that
function getRarityChanceNum(min, max, rarity) {
  var val = 0;
  for (var i = 1; i <= rarity; i++) {
    val += getRarityChance(min, max, i);
  }
  console.log("Pull rate for " + rarity + " with min " + min + " and max " + max + " is " + val);
  return val;
}

// given a unit, pulls a rarity based on chanceRates
function getRarity(name) {
  var min_rarity = chars[name].min_rarity;
  var max_rarity = chars[name].max_rarity;
  var rng = Math.random();
  console.log("RNG: " + rng)
  for (var i = 0; i < rarities.length; i++) {
    if (rng < getRarityChanceNum(min_rarity,max_rarity,rarities[i])) {
      return i+1;
    }
  }
}
...
function getATK(name, level, rarity, specialization) {
  var template_char = chars[name];
  var val = baseStats.atk[template_char.class];
  val += (rarity/2) * baseStats.atk[template_char.class];
  val += statsPerLevel.atk[rarity] * level;
...
function getDEF(name, level, rarity, specialization) {
  var template_char = chars[name];
  var val = baseStats.def[template_char.armor];
  val += (rarity/2) * baseStats.def[template_char.armor];
  val += statsPerLevel.def[rarity] * level;
  val *= template_char.def_modifier;
  return Math.ceil(val);
}

function getHP(name, level, rarity, specialization) {
  var template_char = chars[name];
  var val = baseStats.hp[template_char.class];
  val += (rarity/2) * baseStats.hp[template_char.class];
  val += statsPerLevel.hp[rarity] * level;
  val *= template_char.hp_modifier;
  return Math.ceil(val);
}

function getSPD(name, level, rarity, specialization) {
  var template_char = chars[name];
  var val = baseStats.spd[template_char.class];
  val += (rarity/2) * baseStats.spd[template_char.class];
  val += statsPerLevel.spd[rarity] * level;
...
module.exports.char = class {
  constructor(rateUps = [], ownerId) {
    ...
    this.min_rarity = char.min_rarity;
    this.max_rarity = char.max_rarity;
    ...
    var rarity = getRarity(this.unit_name);
    this.rarity = rarity;
    this.lvl = 0;
    this.atk = getATK(this.unit_name,this.lvl,this.rarity,this.specialization);
    this.def = getDEF(this.unit_name,this.lvl,this.rarity,this.specialization);
    this.hp = getHP(this.unit_name,this.lvl,this.rarity,this.specialization);
    this.spd = getSPD(this.unit_name,this.lvl,this.rarity,this.specialization);
  }
}
